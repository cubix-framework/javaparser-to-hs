...
public class SimpleConsole extends JPanel
  ...
  public SimpleConsole(GruntspudContext context) {
    ...
    toolBar.add(UIUtil.createButton(clearAction = new ClearAction(),
        showSelectiveText, false));
    toolBar.add(UIUtil.createButton(copyAction = new CopyAction(),
        showSelectiveText, false));
    toolBar.add(UIUtil.createButton(saveAction = new SaveAction(),
        showSelectiveText, false));
    ...
    gbc.weighty = 0.0;
    gbc.weightx = 0.0;
    UIUtil.jGridBagAdd(middlePanel, memoryGauge, gbc,
    ...
    // Listen for commands started and stopping
    CVSCommandHandler.getInstance().addGruntspudCVSListener(
        new SimpleConsoleCVSListener());
   ...
   */
  public void init() {
    textPane.setBackground(ColorUtil.getColor(
        JDK13GruntspudHost.PROP_CONSOLE_BACKGROUND, UIManager
            .getColor("TextPane.background"), context));

    Font f = StringUtil.stringToFont(context.getHost().getProperty(
        JDK13GruntspudHost.PROP_CONSOLE_FONT, "monospaced,0,10"));

    if (f != null) {
      textPane.setFont(f);

    }
    maxLength = context.getHost().getIntegerProperty(
        JDK13GruntspudHost.PROP_CONSOLE_MAX_SIZE, 131072);
    checkMaxLength(0);
    memoryGauge.setVisible(context.getHost().getBooleanProperty(
        JDK13GruntspudHost.PROP_SHOW_MEMORY_MONITOR, false));
  }
  ...
  public void writeToStatusLine(Color c, String message, Icon icon) {
    ...
    statusLabel.setForeground((c == null) ? UIManager
        .getColor("Label.foreground") : c);
  ...
  public void writeToInfoLine(Color c, String message, Icon icon) {
    ...
    infoLabel.setForeground((c == null) ? UIManager
        .getColor("Label.foreground") : c);
  ...
  public void saveConsole() {
    ...
    String dir = context.getHost().getProperty(
        JDK13GruntspudHost.PROP_CONSOLE_LAST_SAVE_LOCATION);
  ...
  public void writeMessage(TextStyle style, final String text) {
    ...
    /*
     * synchronized (textPane) { StringBuffer buf = new StringBuffer(text);
     * 
     * if (!text.endsWith("\n")) { buf.append('\n'); } final String t =
     * buf.toString(); final SimpleAttributeSet attr = new SimpleAttributeSet();
     * if (style.getForeground() != null) StyleConstants.setForeground(attr,
     * style.getForeground()); else StyleConstants.setForeground(attr,
     * textPane.getForeground()); if (style.getBackground() != null)
     * StyleConstants.setBackground(attr, style.getBackground()); else
     * StyleConstants.setBackground(attr, textPane.getBackground());
     * StyleConstants.setBold(attr, style.isBold());
     * StyleConstants.setItalic(attr, style.isItalic());
     * SwingUtilities.invokeLater(new Runnable() { public void run() { try {
     * textPane.getDocument().insertString(textPane.getDocument().getLength(),
     * t, attr); textPane.scrollRectToVisible(textPane.getVisibleRect());
     * checkMaxLength();
     * textPane.setCaretPosition(textPane.getDocument().getLength()); } catch
     * (Throwable ex) { }
     *  } }); }
     */
  }
  ...
  public void setCommandRunning(boolean commandRunning) {
    ...
    if (was != commandRunning) {
      if (commandRunning) {
        ...
        int ms = took;
        writeMessage(context.getTextStyleModel().getStyle(
            Constants.OPTIONS_STYLE_GRUNTSPUD), "Command took " + h + "h" + m
            + "m" + s + "s" + ms + "ms");
      }
  ...
  class CopyAction extends AbstractCopyAction {
    ...
    public void copy() {
      try {
        ...
        c.setContents(new StringSelection(textPane.getText().substring(
      ...
      while (true) {
        ...
        while (consoleBuffer.size() > 0) {
          ...
          if (lastStyle == null || lastStyle != w.style) {
            ...
            if (w.style.getForeground() != null)
              StyleConstants.setForeground(attr, w.style.getForeground());
            else
              StyleConstants.setForeground(attr, textPane.getForeground());
            if (w.style.getBackground() != null)
              StyleConstants.setBackground(attr, w.style.getBackground());
            else
              StyleConstants.setBackground(attr, textPane.getBackground());
          ...
          public void run() {
            try {
              ...
              textPane.getDocument().insertString(
                  textPane.getDocument().getLength(), writeTxt, attr);
