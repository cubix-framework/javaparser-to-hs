...
public class LipstikBorderFactory
{
    ...
    public static void paintRoundBorder(Graphics g, int x, int y, int width, int height, LipstikColorTheme theme, Color back, int status)
    {
        ...
        // outer flat border
        if ((status & BORDER_DISABLED) > 0)
            g.setColor(theme.getBorderDisabled());
        else
        	if ((status & BORDER_BRIGHTER) >0)
    ...
    private static class ComboFlatBorder extends AbstractBorder implements UIResource
    {
        ...
        public void paintBorder (Component c, Graphics g, int x, int y, int width, int height)
        {
    		...
    		paintComboBorder(g, x, y, width, height, theme, theme.getControl(), BORDER_FLAT,
    ...
    public static class OptionalMatteBorder implements Border
    {
    	...
    	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height)
    	{
            ...
            // TODO: a bit foolish but how to solve that in other way?
            boolean isSelected;

            if (parent instanceof LipstikInternalFrameTitlePane)
                isSelected = ((LipstikInternalFrameTitlePane)parent).getFrame().isSelected();
            else
                isSelected = ((LipstikTitlePane)parent).getWindow().isActive();
    ...
    public static class PopupMenuBorder extends AbstractBorder implements UIResource
    {
        ...
        public void paintBorder (Component c, Graphics g, int x, int y, int width, int height)
        {
            ...
            if (style == MENUBAR_SHADOW)
            	LipstikGradients.drawGradient(g, theme.getMenuGradient(), theme.getMenuBackground(), 1, 1, ICON_BAR_WIDTH, height-2, false);
            else
            if (style == MENUBAR_SHARP)
            	g.fillRect(1, 1, ICON_BAR_WIDTH, height-2);
        }
    }
