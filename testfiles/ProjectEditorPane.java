...
//		try {
//			gbc.weightx = 0.0;
//			UIUtil.jGridBagAdd(p, new JLabel("Filter"), gbc, GridBagConstraints.RELATIVE);
//			gbc.weightx = 1.0;
//			filterModel = (CVSFileFilterModel) context.getFilterModel().clone();
//			UIUtil.jGridBagAdd(p, defaultFilter = new JComboBox(filterModel), gbc, GridBagConstraints.REMAINDER);
//		}
...
//		}
		...
		UIUtil.jGridBagAdd(
			p,
			home = new FileNameTextField(null, "", 15, true, false, chooser, false),
			gbc,
		...
		description =
			new MiniTextEditor(
				...
				false,
				false,
				Constants.PROJECT_DESCRIPTION_EDITOR_WORD_WRAP,
				true,
			...
			if (!f.exists()) {
				int opt =
					JOptionPane.showConfirmDialog(
						this,
						"The home directory does not exist. Do you wish to create it?",
...
//		CVSFileFilter filter =
//			project.getDefaultFilter() == null
//				|| project.getDefaultFilter().equals("") ? null : filterModel.getFilter(project.getDefaultFilter());
//		filterModel.setSelectedItem(filter == null ? filterModel.getElementAt(0) : filter);
	}
}
